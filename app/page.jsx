    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group" style={{ marginBottom: 24 }}>
            <label className="muted" style={{ display: 'block', marginBottom: 8, fontSize: '14px' }}>
              首次买入日期 <span style={{ color: 'var(--danger)' }}>*</span>
            </label>
            <div
...
export default function HomePage() {
  ...
  const processPendingQueue = async () => {
    ...
    for (const trade of currentPending) {
      ...
      if (result && result.value > 0) {
        ...
        const nextHoldingDate = newShare > 0
          ? (trade.type === 'buy' ? (current.holdingDate ?? trade.date ?? null) : (current.holdingDate ?? null))
          : null;
        tempHoldings[trade.fundCode] = { ...current, share: newShare, cost: newCost, holdingDate: nextHoldingDate, days: newShare > 0 ? current.days ?? null : null };
        stateChanged = true;
  ...
  const handleTrade = (fund, data) => {
    ...
    const nextHoldingDate = newShare > 0
      ? (isBuy ? (current.holdingDate ?? data.date ?? null) : (current.holdingDate ?? null))
      : null;
    handleSaveHolding(fund.code, { share: newShare, cost: newCost, holdingDate: nextHoldingDate });
    setTradeModal({ open: false, fund: null, type: 'buy' });
  };
