...
-- ============================================================================
-- fund_topic 表 - 板块资金流向数据（由浏览器端 sectorFundFlowSync.js 写入）
-- ============================================================================
...
--   net_inflow numeric null,
--   constraint fund_topic_pkey primary key (id)
-- ) TABLESPACE pg_default;

-- 1. 开启 RLS
ALTER TABLE fund_topic ENABLE ROW LEVEL SECURITY;

-- 2. 已登录用户可查询
CREATE POLICY "Allow authenticated users to select fund_topic"
ON fund_topic
FOR SELECT
TO authenticated
USING (auth.uid() IS NOT NULL);

-- 3. 已登录用户可插入（给 sectorFundFlowSync 写入用）
CREATE POLICY "Allow authenticated users to insert fund_topic"
ON fund_topic
FOR INSERT
TO authenticated
WITH CHECK (auth.uid() IS NOT NULL);

-- 4. 已登录用户可删除（给 sectorFundFlowSync 清理旧数据用）
CREATE POLICY "Allow authenticated users to delete fund_topic"
ON fund_topic
FOR DELETE
TO authenticated
USING (auth.uid() IS NOT NULL);
